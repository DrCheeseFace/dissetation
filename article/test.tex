...
\cite{honaker_amelia_2024}, Profiler; but these applications focus on analyzing
...
imputation methods and compare the results of each.

% TODO write about seems to lack researched visualisation methods  
% TODO write about can only compare the immediatly created versions 
% TODO cannot compare eg against unimputed 
...
\begin{description}
        \item [Wasserstein Distance] {\displaystyle W_{p}(\mu ,\nu )=\inf _{\gamma \in \Gamma (\mu ,\nu )}\left(\mathbf {E} _{(x,y)\sim \gamma }d(x,y)^{p}\right)^{\frac {1}{p}},} ~\\
        \item [Kolmogorov-Smirnov Test] {\displaystyle F_{n}(x)={\frac {{\text{number of (elements in the sample}}\leq x)}{n}}={\frac {1}{n}}\sum _{i=1}^{n}1_{(-\infty ,x]}(X_{i}),}
        \item [Variance Ratio] \displaystyle {VAR\_RATIO} = \frac{\sigma^2_{\text{child}}}{\sigma^2_{\text{base}}}
        \item [Skew Difference] \displaystyle \Delta \text{Skew} = \text{Skew}_{\text{base}} - \text{Skew}_{\text{child}}
\end{description}

\textbf{Wasserstein Distance} is a metric to describe how similar two
distributions are to each other. In the example a user performs an imputation
...
\textbf{Kolmogorov-Smirnov Test} similar to the Wassserstein Distance, it
...
\textbf{Variance Ratio} in the context of comparing different imputations of a
dataset shows whether one imputation method is smoothing the data more than the
other. This is useful when evaluating the risk of information loss and the
preservation of natural noise within the data. If we see a variance ratio
significantly below one, this may indicate that one of the imputation methods
has artificially collapsed the distribution. This can be used to verify if the
other metrics provided are worth looking at due to lower standard error.

\textbf{Skew Difference} is the simplest to interpret out of the four. This
shows the direction and magnitude of the skew for a specified feature. 
...
the performed imputations similar to the other implementations of this in \mica.
