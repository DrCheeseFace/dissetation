...
MICA is split into 3 software components \cref{fig:MICA architecture}. 
\begin{itemize}
        \item MICA API 
        \item Impute Handler
...
\textit{MICA} provides a simple-to-use API for uploading, imputing and managing
versions of imputed datasets. This is routed through an NGINX reverse proxy to provide in
the future RESTful API version sub-routing. The MICA API handles the imputation 
versioning system. This acts as the bridge between the frontend client and the
impute handler.
...
Scikit-learn imputation library was picked for use in \textit{MICA} as the core
...
manner in the frontend, \textit{MICA} uses React  \cite{noauthor_react_nodate}
+ MobX  \cite{noauthor_about_nodate} to cache results requested from the MICA
API. For low latency interactability, \textit{MICA} uses Shadcn-ui
...
For example, the below node diagram depicts the history of imputations
...
To store the history of a data imputation, \textit{MICA} uses a hierarchical
